...
async def my_questionnaire(message: Message, bot: Bot):
    await bot.send_message(message.from_user.id, f'Ваша анкета: \n'
...
async def anceta_step_one(message: Message, bot: Bot,  state: FSMContext):
    await bot.send_message(message.from_user.id,'Введите свое имя. Можно Ф.И.О')
...
async def anceta_step_two(message: Message, bot: Bot, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, 'Введите свою дату рождения. Формат: ДД.ММ.ГГГГ')
...
async def anceta_step_three(message: Message, bot: Bot, state: FSMContext):
    # Сохраняем возраст пользователя в словаре
    await state.update_data(birthday=message.text)
    await bot.send_message(message.from_user.id, 'Введите свой номер телефона. Формат: +7-ХХХ-ХХХ-ХХ-ХХ/8-XXX-XXX-XX-XX')
...
async def anceta_step_four(message: Message, bot: Bot,  state: FSMContext):
    # Сохраняем номер телефона пользователя в словаре
    await state.update_data(phone_number=message.text)
    await bot.send_message(message.from_user.id, 'Анкета заполнена.')
    ...
    await bot.send_message(message.from_user.id, f'Анкета изменена: \n'
