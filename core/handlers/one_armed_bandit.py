...
async def get_game(message: Message, bot: Bot):
    if UsersSQL().coins_count(message.from_user.id)[0][0] == 0:
        await bot.send_message(message.from_user.id, f'У тебя нету монет, нищеброд. Сыграй в "Угадайку", может подзаработаешь.',
